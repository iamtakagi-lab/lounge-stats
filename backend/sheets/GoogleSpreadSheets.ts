...
export default class GoogleSpreadSheets {
    ...
    private auth = async (): Promise<gapis.sheets_v4.Sheets> => {
        const client = await gapis.google.auth.getClient({
            ...
            keyFile: './credentials/spreadsheet-keys.json'
        });
    ...
    fetchValueByCoords = async (coordA: string, coordB: string): Promise<gapis.sheets_v4.Schema$ValueRange> => {
        const authed = await this.auth();
        const res = await authed.spreadsheets.values.get({ spreadsheetId: this.spreadsheetId, range: this.tabTitle+ "!"+ coordA + ":" + coordB });
        return res.data;
    }
